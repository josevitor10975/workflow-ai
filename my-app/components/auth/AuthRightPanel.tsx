...
export function AuthRightPanel() {
  return (
    <aside className="relative hidden min-h-full overflow-hidden rounded-lg bg-card text-card-foreground lg:flex lg:flex-col lg:justify-center">
      <div aria-hidden="true" className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-primary/5 blur-3xl" />
      <div aria-hidden="true" className="pointer-events-none absolute -bottom-32 -left-24 h-64 w-64 rounded-full bg-primary/[0.03] blur-3xl" />
      ...
      <div className="relative px-8 py-10 sm:px-12 sm:py-12 lg:px-16 lg:py-14">
        <div className="mb-10 max-w-lg">
          ...
          <p className="mt-3 max-w-md text-sm leading-6 text-muted-foreground">
            A focused workflow for comparing AI outputs, making informed decisions, and turning the best result into real development work.
          </p>
        </div>

        <div className="relative max-w-xl">
          <div aria-hidden="true" className="absolute bottom-7 left-[17px] top-7 w-px bg-border" />
          <div className="relative space-y-2">
            {steps.map((step, index) => {
              ...
              return (
                <div key={step.number} className="group relative flex gap-4 rounded-xl py-2.5 transition-colors duration-200 hover:bg-background/60 sm:gap-5 sm:px-3">
                  ...
                  <div className={`min-w-0 flex-1 ${isLast ? "pb-1" : "pb-5"}`}>
                    <div className="flex items-baseline gap-2.5">
                      <span className="font-mono text-[10px] font-medium tracking-widest text-muted-foreground/70">{step.number}</span>
                      <h3 className="font-serif text-base font-medium text-foreground">{step.title}</h3>
                    </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="font-serif text-sm font-medium text-foreground">More than a benchmark</p>
                <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
              </div>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Compare. Evaluate. Choose. Build.</p>
            </div>
          </div>
        </div>
      </div>
