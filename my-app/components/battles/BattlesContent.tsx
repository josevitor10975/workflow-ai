...
function ProgressRing({ value }: { value: string }) {
  ...
  return (
    <div className="relative flex size-14 items-center justify-center">
      ...
      <span className="relative text-sm font-semibold">{value}</span>
    </div>
  );
}
  ...
  return (
    <main className="flex flex-1 flex-col px-4 py-8 md:px-8 md:py-12 lg:px-14">
      <div className="mx-auto w-full max-w-[1400px]">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <section className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-lg leading-8 text-muted-foreground">
              Track and manage your latest AI experiments. Compare results,
              evaluate artifacts and choose the best outcome for your workflow.
            </p>
          </section>
        ...
        <div className="mt-10 hidden grid-cols-[1.55fr_1.2fr_1fr_0.85fr_1fr_36px] gap-4 px-4 text-sm font-medium text-muted-foreground xl:grid">
          <span>Battle</span>
          <span>Workspace</span>
          <span>Models</span>
          <span>Status</span>
              ...
              <CardContent className="grid items-center gap-5 p-4 xl:grid-cols-[1.55fr_1.2fr_1fr_0.85fr_1fr_36px]">
                <div className="flex min-w-0 items-center gap-4">
                  <span className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-muted text-foreground">
                    <FontAwesomeIcon icon={battle.icon} className="size-5" />
                  </span>
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Created {battle.created} · {battle.models.length} models
                    </p>
                  </div>
                </div>
                <div className="min-w-0">
                ...
                <div className="space-y-1">
                  {battle.models.map((model) => (
                    <p key={model} className="flex items-center gap-2 text-sm">
                      <span className="flex size-4 shrink-0 items-center justify-center rounded bg-muted text-[9px] font-semibold text-foreground">
                        {model === "Gemini 1.5" ? "G" : "AI"}
                      </span>
                      {model}
                    </p>
                  ))}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="hidden min-w-0 sm:block">
                    ...
                    {battle.score !== "—" ? (
                      <>
                        <p className="mt-1 text-sm font-medium">Claude 3.5</p>
                ...
                <Button
                  ...
                  aria-label={`Actions for ${battle.name}`}
                >
