...
const battles = [
  ...
  },
  {
    ...
    description: "Create a user dashboard with key metrics and recent activity.",
    models: ["GPT-5.6", "Gemini 2.5", "Claude 4.0"],
    status: "In progress",
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-8 lg:px-10">
      ...
      <section className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2 lg:pt-8">
          ...
          <Button variant="outline" size="icon" aria-label="Workspace actions">
            ...
            <button
              ...
              className={`relative shrink-0 pb-3 text-sm font-medium ${
                tab === "Battles"
                  ...
                  : "text-muted-foreground disabled:cursor-default"
              }`}
            >
              {tab}
            </button>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Compare experiments and choose the best result for this workspace.
            </p>
          </div>
            ...
            >
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(180px,1fr)_minmax(170px,.7fr)_minmax(170px,.65fr)_28px] lg:items-center">
                <div className="flex min-w-0 gap-4">
                  ...
                  <div className="min-w-0">
                    <h3 className="truncate text-sm font-medium md:text-base">{battle.title}</h3>
                      ...
                      {battle.models.map((model) => (
                        <span key={model} className="inline-flex items-center gap-1.5">
                          <span className="flex size-5 items-center justify-center rounded-full bg-muted text-[9px] font-semibold text-foreground">
                            {model === "GPT-5.6" ? "G" : model === "Gemini 2.5" ? "✦" : "C"}
                          </span>
                          {model}
                        </span>
                      ))}
                ...
                <div className="min-w-0 border-border lg:border-l lg:pl-5">
                  {battle.winner ? (
                    <>
                      ...
                      <p className="mt-1 truncate text-sm font-medium">{battle.winner}</p>
                    </>
                  ) : (
                    <>
                      <p className="text-xs text-muted-foreground">Progress</p>
                      <p className="mt-1 text-sm font-medium">{battle.progress}</p>
                    </>
                  )}
                </div>
                  ...
                  {battle.score ? (
                    <>
                      ...
                      <p className="mt-1 text-lg font-medium">
                        {battle.score} <span className="text-sm text-muted-foreground">/ 10</span>
                      </p>
                    </>
                  ) : (
                ...
                <Button variant="ghost" size="icon-sm" aria-label={`Open ${battle.title}`}>
